...
def validate(doc, method):
	...
	doc.eos_amount = eos_amount(doc.get('applicant'), doc.get('required_by_date'), doc.get('company'))
...
def get_balance_loan(applicant):
	loan = """select sum(rps.total_payment) as 'total_loan'
		...
		and l.docstatus = 1
		and l.applicant = '{}'
		group by l.applicant""".format(applicant)
...
def eos_amount(applicant, required_date, company):
	...
	return (first_slab * (net_pay / 2)) + ((second_slab * net_pay) + ((net_pay / 12) * months))
...
def validate_employee(applicant, required_date):
	if not applicant or not required_date: return
	
	joining_date, relieving_date = frappe.get_cached_value("Employee", applicant,
				["date_of_joining", "relieving_date"])
	
	if getdate(required_date) < getdate(joining_date):
		frappe.throw(_('Loan Application can only be applied after {}').format(formatdate(joining_date)))
	
	if relieving_date and getdate(required_date) > getdate(relieving_date):
		frappe.throw(_('Loan Application cannot be applied after {}').format(formatdate(relieving_date)))