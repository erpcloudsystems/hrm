...
def get_employee_shift(employee, for_date=nowdate(), consider_default_shift=False, next_shift_direction=None):
	...
	shift_type_query="select shift_type from `tabShift Assignment` where docstatus=1 and employee='{0}' and '{1}' between start_date and end_date".format(employee,for_date)
	...
	if not shift_type_name and next_shift_direction:
		...
		if consider_default_shift and default_shift:
			...
			dates_query="select start_date from `tabShift Assignment` where docstatus=1 and employee='{0}' and  start_date {4} '{1}' order by start_date {2} limit {3}".format(employee,for_date,sort_order,MAX_DAYS,direction)
