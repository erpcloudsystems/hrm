...
def validate(doc,method):
	sal_str="select * from `tabSalary Slip` where '"+str(doc.start_date)+"' >= actual_attendance_start_date and '"+str(doc.end_date)+"' <= actual_attendance_end_date and employee='"+doc.employee+"' and docstatus=1"
	...
	att_str="select * from `tabLeave Request` where '"+str(doc.start_date)+"' >= from_date and '"+str(doc.end_date)+"' <= to_date and employee='"+doc.employee+"' and docstatus=1 and status='Approved'"
	att_sql=frappe.db.sql(att_str,as_dict=True)
	if att_sql:
		att_doc=frappe.get_doc("Leave Request",att_sql[0].name)
		if att_doc.name:
			frappe.throw("Cannot create Timesheet Leave applied from "+str(att_doc.from_date)+" - "+str(att_doc.to_date)+" Leave Request <a href='#Form/Leave Request/"+att_doc.name+"'>"+att_doc.name+"</a>")

	app_str="select * from `tabLeave Application` where '"+str(doc.start_date)+"' >= from_date and '"+str(doc.end_date)+"' <= to_date and employee='"+doc.employee+"' and docstatus=1 and (workflow_state='Approved' or status='Approved')"
	app_sql=frappe.db.sql(app_str,as_dict=True)
	if app_sql:
		app_doc=frappe.get_doc("Leave Application",app_sql[0].name)
		if app_doc.name:
			frappe.throw("Cannot create Timesheet Leave applied from "+str(app_doc.from_date)+" - "+str(app_doc.to_date)+" Leave Application <a href='#Form/Leave Application/"+app_doc.name+"'>"+app_doc.name+"</a>")

	vac_str="select * from `tabVacation Leave Application` where '"+str(doc.start_date)+"' >= from_date and '"+str(doc.end_date)+"' <= to_date and employee_id='"+doc.employee+"' and docstatus=1 and workflow_state='Approved'"
	vac_sql=frappe.db.sql(vac_str,as_dict=True)
	if vac_sql:
		vac_doc=frappe.get_doc("Vacation Leave Application",vac_sql[0].name)
		if vac_doc.name:
			frappe.throw("Cannot create Timesheet Leave applied from "+str(vac_doc.from_date)+" - "+str(vac_doc.to_date)+" Vacation Leave Application <a href='#Form/Vacation Leave Application/"+vac_doc.name+"'>"+vac_doc.name+"</a>")
	...
	if doc.workflow_state=="Approved":
		...
		for i in doc.time_logs:
			if i.confirm_yes_no=="Yes":
				sql="select * from `tabEmployee Checkin` where employee='"+doc.employee+"' and `tabEmployee Checkin`.`time` between '"+i.from_time+"' and '"+i.to_time+"'"
		...
		att_str="select * from `tabAttendance` where cast(attendance_date as date) between cast('"+str(doc.start_date)+"' as date) and cast('"+str(doc.end_date)+"' as date) and employee='"+doc.employee+"' and docstatus=1"
		att_sql=frappe.db.sql(att_str,as_dict=True)
...
def on_cancel(doc,method):
	...
	emp_check="select * from `tabEmployee Checkin` where timesheet_reference='"+doc.name+"'"
...
def get_chekin_list(doc):
	for i in doc.time_logs:
		...
		if str_sql:
			...
			for empi in str_sql:
				if htl_str1=="":
					htl_str1=html_str+"<tr><td style='border: 1px solid black;padding: 5px;text-align:center;'>"+str(empi.log_type)+"</td><td style='border: 1px solid black;padding: 5px;text-align:center;'>"+str(empi.time)+"</td></tr>"
				else:
					htl_str1=htl_str1+"<tr><td style='border: 1px solid black;padding: 5px;text-align:center;'>"+str(empi.log_type)+"</td><td style='border: 1px solid black;padding: 5px;text-align:center;'>"+str(empi.time)+"</td></tr>"
...
def set_check(employee,from_time,to_time):
	...
	sql="select * from `tabEmployee Checkin` where employee='"+employee+"' and `tabEmployee Checkin`.`time` between '"+from_time+"' and '"+to_time+"'"
	str_sql=frappe.db.sql(sql,as_dict=True)
	html_str="<center><table style='border: 1px solid black;width:100%'><tr ><th style='border: 1px solid black;padding: 5px;text-align:center;'>Log Type</th><th style='border: 1px solid black;padding: 5px;text-align:center;'>Time</th><tr>"
	if str_sql:
		htl_str1=""
