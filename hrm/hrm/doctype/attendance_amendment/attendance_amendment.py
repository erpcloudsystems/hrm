...
from erpnext.hr.doctype.employee_checkin.employee_checkin import time_diff_in_hours
...
class AttendanceAmendment(Document):
	...
	def processed_attendance(self, reset=1):
		...
		if att.get('name'):
			punch = """SELECT *
				...
				WHERE attendance = '{}'
				ORDER BY time""".format(att.get('name'))
		...
		if att.get('amended_request'):
			amended_att = """SELECT *
				...
				WHERE docstatus = 1
				AND name = '{}'""".format(att.get('amended_request'))
	...
	def cal_working_hrs(self):
		start_date_time = '{} {}'.format(self.get('attendance_date'), self.get('amended_in_time'))
		...
		working_hrs = time_diff_in_hours(get_datetime(start_date_time), get_datetime(end_date_time))
