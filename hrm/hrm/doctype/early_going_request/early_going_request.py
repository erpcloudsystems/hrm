...
class EarlyGoingRequest(Document):
	...
	def validate(self):
		if self.is_new():
			if float(self.actual_number_of_minutes_late or 0) != 0 :
				self.approved_number_of_minutes_late = self.actual_number_of_minutes_late
			else:
	...
	def on_submit(self):
		...
		if self.get('status') == "Open":
			frappe.throw(_("Only Early Going Request with status 'Approved' and 'Rejected' can be submitted"))
	...
	def validate_date(self):
		salary_slip = """SELECT `S`.*
			...
			WHERE `S`.`docstatus` < 2
			AND `S`.`employee` = '{}'
			AND '{}' BETWEEN `PP`.`actual_start_date` and `PP`.`actual_end_date`""".format(self.get('employee'), self.get('attendance_date'))
	...
	def validate_duplicate(self):
		duplicate = """SELECT *
			...
			AND employee = '{}'
			AND attendance_date = '{}'
			AND name != '{}'""".format(self.get('employee'), self.get('attendance_date'), self.get('name'))
		duplicate = frappe.db.sql(duplicate, as_dict=True)

		if duplicate:
			frappe.throw(_("Employee has Already Applied for Early Going Request for this date {}".format(formatdate(self.get('attendance_date')))))
	...
	def att_application(self, validate):
		...
		attendance = """SELECT *
			...
			WHERE docstatus = 1
			AND employee = '{}'
			AND attendance_date = '{}'""".format(self.get('employee'), self.get('attendance_date'))
		...
		if not attendance.get('name'):
			...
			if attendance.get('status') != 'Present':
				...
				self.reason_for_late_coming = (self.reason_for_late_coming or "") + " Employee is not Present"
				return
	...
	def employee_checkin(self):
		...
		from hrm.custom_script.attendance.attendance import early_going

		current_shift = get_employee_shift(employee=self.get('employee'), for_date=getdate(self.get('attendance_date')), consider_default_shift=True, next_shift_direction='forward')
		...
		logs = """SELECT *
			...
			AND shift_end = '{}'
			AND shift = '{}'
			ORDER BY employee, time""".format(self.get('employee'), current_shift.start_datetime, current_shift.end_datetime, current_shift.shift_type.name)
