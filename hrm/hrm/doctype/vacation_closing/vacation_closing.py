...
class VacationClosing(Document):
	...
	def on_cancel(self):
		...
		if self.employee_loan_reference :
			...
			if canceller.docstatus == 1:
				canceller.cancel()
		vacation_doc = frappe.get_doc("Vacation Leave Application", self.leave_application_reference)
	...
	def on_trash(self):
		...
		if self.employee_loan_reference :
			try:
				sql = " UPDATE `tabVacation Closing` SET employee_loan_reference = null "
	...
	def validate_grand_total(self):
		self.grand_total = flt(self.present_day_salary) + flt(self.salary_processed_amount) + flt(self.advance_amount) - flt(self.different_ticket)
	...
	def mandatory_fild(self):
		if not self.mode_of_payment or not self.payment_account or not self.employee_loan_account or not self.interest_income_account:
			frappe.throw("Please fill the mandatory field ['Mode of Payment','Payment Account','Employee Loan Account','Interest Income Account']")
	

	def validate_salary_slip_date(self):
		if self.salary_start_date and self.leave_from and getdate(self.salary_start_date) != get_first_day(self.leave_from):
			frappe.throw("Salary Start Date Cannot be Less than Month First Date")
		
		if self.salary_end_date and self.leave_from and getdate(self.salary_end_date) >= getdate(self.leave_from):
			frappe.throw("Salary End Date Cannot be Greater than or Equal to Leave From Date")
		
		if self.salary_start_date and self.salary_end_date and getdate(self.salary_end_date) < getdate(self.salary_start_date):
	...
	def get_sal_and_attendance(self):
		...
		try:
			...
			if not float(doc.payment_days) < 0 and self.enable_salary_date == 1 and self.salary_end_date != None:
	...
	def set_val_inchild(self, target_field, source_child, vacation=False):
		for row in source_child:
			...
			if flag:
				pass
			elif row.get('vacation_amount' if vacation else 'amount') >= 0 or row.get('default_amount') > 0:
	...
	def get_default_cost_center(self):
		cost_center = frappe.db.get_value("Company",
			{"company_name": self.company}, "cost_center")

		if not cost_center:
			frappe.throw("Please set Default Cost Center in Company {0}".format(self.company))

		return cost_center


	def get_salary(self):
		query = "select base from `tabSalary Structure Employee` where employee ='"+self.employee+"'"
	...
	def get_jv(self):
		query =  "select name from `tabJournal Entry` where doctype_id='"+self.name+"';"
		data = frappe.db.sql(query, as_dict=True)
		if data:
			return data[0]['name']
		else:
	...
	def get_attendance(self):
		...
		query ="select count(*) 'attendance' from `tabAttendance` where employee = '"+self.employee+"' and status = 'Present' and attendance_date >= '"+str(month_start_date)+"' and attendance_date <='"+str(month_end_date)+"'"
		data = frappe.db.sql(query, as_dict=True)
		if data:
			return data
	...
	def vacation_leave_save(self):
		...
		vacay_app = frappe.get_doc("Vacation Leave Application",self.leave_application_reference)
	...
	def has_jv_entries(self):
		...
		journal_entries = frappe.db.sql("""SELECT J.name
			...
			AND J.doctype_id = '{1}'""".format(self.doctype, self.name), as_dict=1)
	...
	def last_vacation(self):
		...
		self.last_rejoining_date = last_vec_res[0]['vacation_rejoining_date'] if last_vec_res else self.joining_date
...
def get_leave_application(doctype, txt, searchfield, start, page_len, filters):
	emp_sql = """SELECT `VLA`.`name`, `VLA`.`employee_id`, `VLA`.`employee_name`
		...
		OR `VLA`.`employee_name` LIKE %(txt)s)""".format(filters.get("company"), filters.get("name"))
	
	return frappe.db.sql(emp_sql, {
			'txt': "%%%s%%" % txt
		})