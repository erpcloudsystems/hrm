...
def execute(filters=None):
	...
	frappe.errprint("holiday_list>"+str(holiday_list))
	...
	data = []
	leave_types = frappe.db.sql("""select name,leave_type_abbr from `tabLeave Type`""", as_list=True)
	leave_list = [d[0] for d in leave_types]
	columns.extend(leave_list)
	columns.extend([_("Total Late Entries") + ":Float:120", _("Total Early Exits") + ":Float:120"])
	leave_types1 = frappe.db.sql("""select name,leave_type_abbr from `tabLeave Type`""", as_dict=True)
	abbr=["H","WO"]
	...
	for emp in sorted(att_map):
		...
		for day in range(filters["total_days_in_month"]):
			...
			status_map = {"Present": "P", "Absent": "A", "Half Day": "HD", "On Leave": "L", "None": "", "Holiday":"PH","Week Off":"WO", "Holidays":"<b>PH</b>","Week Offs":"<b>WO</b>"}
			...
			if status == "Present" or status == "Week Offs" or status == "Holidays":
				total_p += 1
			elif status == "Absent" or status == "Week Off" or status == "Holiday":
				total_a += 1
...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		case when ifnull(leave_type,'')='' then status when leave_type='Holiday' and status='Present' then 'Holidays' when leave_type='Week Off' and status='Present' then 'Week Offs' else status end as 'status'
		from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
		conditions, filters, as_dict=1)
    
	frappe.errprint(str(attendance_list))
	att_map = {}
